...
pub const STDIN: *mut c_void = 0 as *mut c_void;
pub const STDOUT: *mut c_void = 1 as *mut c_void;
pub const STDERR: *mut c_void = 2 as *mut c_void;
...
pub unsafe extern "C" fn fgetc(st: *mut c_void) -> c_int {
    let mut b = 0u8;
    if fread(&mut b as *mut u8 as *mut c_void, 1, 1, st) == 1 { b as c_int } else { -1 }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn getc(st: *mut c_void) -> c_int { fgetc(st) }
#[unsafe(no_mangle)]
pub unsafe extern "C" fn putc(c: c_int, st: *mut c_void) -> c_int { if fwrite(&c as *const c_int as *const c_void, 1, 1, st) == 1 { c } else { -1 } }
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ungetc(_c: c_int, _st: *mut c_void) -> c_int { _c }
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ferror(_st: *mut c_void) -> c_int { 0 }
#[unsafe(no_mangle)]
...
pub unsafe extern "C" fn fread(p: *mut c_void, s: usize, n: usize, st: *mut c_void) -> usize {
    ...
    let fd = if st as usize == 1 || st as usize == 2 { 1 } else if st as usize == 0 { 0 } else { (*(st as *mut std::fs::File)).as_raw_fd() };
    let res = std::os::file_read(fd, core::slice::from_raw_parts_mut(p as *mut u8, s * n));
    if res != usize::MAX { res / s } else { 0 }
}
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fwrite(p: *const c_void, s: usize, n: usize, st: *mut c_void) -> usize {
    if st.is_null() { return 0; }
    let fd = if st as usize == 1 || st as usize == 2 { 1 } else if st as usize == 0 { 0 } else { (*(st as *mut std::fs::File)).as_raw_fd() };
    let res = std::os::file_write(fd, core::slice::from_raw_parts(p as *const u8, s * n));
    if res != usize::MAX { res / s } else { 0 }
}
#[unsafe(no_mangle)]
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vsprintf(str: *mut c_char, fmt: *const c_char, ap: VaList) -> c_int {
...
pub unsafe extern "C" fn remove(pathname: *const c_char) -> c_int {
    let resolved = crate::misc::resolve_path_rust(pathname);
    if std::os::syscall(73, resolved.as_ptr() as u64, resolved.len() as u64, 0) == 0 { 0 } else { -1 }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn rename(oldpath: *const c_char, newpath: *const c_char) -> c_int {
    let old_res = crate::misc::resolve_path_rust(oldpath);
    let new_res = crate::misc::resolve_path_rust(newpath);
    if std::os::syscall4(74, old_res.as_ptr() as u64, old_res.len() as u64, new_res.as_ptr() as u64, new_res.len() as u64) == 0 { 0 } else { -1 }
}
...
pub unsafe extern "C" fn krake_debug_printf(f: *const c_char, mut args: ...) -> c_int {
    ...
    std::os::debug_print(core::str::from_utf8_unchecked(&buffer));
    res
}
...
pub unsafe fn printf_core(mut output: impl FnMut(u8), fmt: *const c_char, args: &mut VaList) -> c_int {
    ...
    while *p != 0 {
        ...
        match spec as u8 {
            ...
            b'c' => {
                output(args.arg::<c_int>() as u8);
                written += 1;
            }
            b'f' => {
                let _v = args.arg::<c_double>();
                write_padded(&mut output, b"FLOAT", width, false, &mut written);
            }
            b'%' => {
                output(b'%');
                written += 1;
...
fn itoa_signed(n: i64, buf: &mut [u8]) -> usize {
    ...
    } else { itoa(n as u64, buf, 10, false) }
}

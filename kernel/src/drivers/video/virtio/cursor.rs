...
pub fn setup_cursor(phys_ptr: u64, width: u32, height: u32, x: u32, y: u32) {
    ...
    // 1. Create Cursor Resource (Queue 0)
    ...
    // 3. Transfer Data to Host (Queue 0)
    ...
    // Send to Cursor Queue (1) with NO response buffer
    send_command_queue(
        1,
        &[&req_update as *const _ as u64],
        &[core::mem::size_of_val(&req_update) as u32],
        &[], // No input descriptors (response)
        &[],
    );
...
pub fn move_cursor(x: u32, y: u32) {
    let req_move = VirtioGpuUpdateCursor {
        ...
        resource_id: 0, // Resource ID ignored for move
        hot_x: 0,
        hot_y: 0,
        padding: 0,
    };

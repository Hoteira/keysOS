...
// VirtIO Common Constants (Copied/Shared)
...
pub fn init() {
    // Try to find VirtIO Block Device (Legacy 0x1001 or Modern 0x1042)
    // We check for Modern first/primarily based on capabilities
    ...
    unsafe {
        ...
        write_32(common_cfg_ptr.add(OFF_DEVICE_FEATURE_SELECT), 1); // Select High features
        let features_high = read_32(common_cfg_ptr.add(OFF_DEVICE_FEATURE));
        
        let mut driver_features_high = 0;
        if (features_high & 1) != 0 { // VIRTIO_F_VERSION_1
        ...
        write_32(common_cfg_ptr.add(OFF_DRIVER_FEATURE), 0); // No low features needed yet
...
unsafe fn setup_queue(common_cfg: *mut u8, index: u16, notify_base: u64, notify_multiplier: u32) { unsafe {
    ...
    let size: u16 = 32; // Use 32 descriptors
...
pub fn read(lba: u64, _disk: u8, target: &mut [u8]) {
    ...
    // Split into 512 chunks if needed, but VirtIO can handle larger.
    // For simplicity, handle one request.
    ...
    // 2. Data Buffer (Write-only for device)
    // 3. Status (Write-only for device)
    
    let req_phys = &header as *const _ as u64;
    let req_len = core::mem::size_of::<VirtioBlkReqHeader>() as u32;
    
    let buf_phys = target.as_mut_ptr() as u64; // Must be physical addr! Assuming identity mapping or physical buffer passed.
    // NOTE: disk::read receives a pointer. In kernel, we usually work with physical/identity mapped addresses for DMA.
...
unsafe fn send_command(out_phys: &[u64], out_lens: &[u32], in_phys: &[u64], in_lens: &[u32]) { unsafe {
    ...
    for i in 0..in_phys.len() {
        let flags = if i == in_phys.len() - 1 { 2 } else { 2 | 1 }; // WRITE | (NEXT if not last)
        let desc = VirtqDesc {
            addr: in_phys[i],
            len: in_lens[i],
            flags,
    ...
    // Update last descriptor to remove NEXT flag

                    ...
                    for (i, segment) in segments.iter().enumerate() {
                        ...
                        if let Some(infile) = parsed.input_file {
                            ...
                            if let Ok(f) = std::fs::File::open(&path) {
                                ...
                                core::mem::forget(f); // Prevent Drop closing it immediately
                                close_stdin = true;
                            } else {
                        ...
                        if let Some(outfile) = parsed.output_file {
                            ...
                            match res {
                                Ok(f) => {
                                    ...
                                    if parsed.append_mode {
                                        std::os::file_seek(stdout_fd, 0, 2); // SEEK_END
                                    }
                                    core::mem::forget(f);
                                    close_stdout = true;
                                },
                            ...
                            if std::os::pipe(&mut fds) == 0 {
                                ...
                                close_stdout = true;
                                // Read end is kept for next iter
                            } else {
                        ...
                        let is_builtin = match parsed.cmd.as_str() {
                            "cd" | "ls" | "pwd" | "help" | "clear" | "touch" | "mkdir" | "rm" | "mv" | "cp" => true,
                            _ => false
                        };
                        
                        if is_builtin {
                             // Built-ins execute in-process. 
                             // We must swap FDs manually if redirected.
                             // Currently we don't have dup2/swap syscalls for the *current* process easily exposed,
                             // or we'd have to use `task.fd_table` manipulation via a hypothetical syscall.
                             // 
                             // Workaround: We only support simple redirections for built-ins if we implement them using `std::os::file_write`
                             // and passing the FD. But our built-in logic uses hardcoded `STDOUT_FD`.
                             // 
                             // For now, built-ins ignore pipes/redirections unless we refactor them to take `output_fd`.
                             // Let's refactor `execute_builtin` to take output_fd.
                             
                             execute_builtin(&parsed.cmd, &parsed.args, &mut cwd, stdin_fd, stdout_fd);
                             
                             // If this was part of a pipe (unlikely useful for cd, but ls | grep is useful), 
                             // we just wrote to the pipe.
                        } else {
                            // External Binary
...
fn execute_builtin(cmd: &str, args: &[String], cwd: &mut String, in_fd: usize, out_fd: usize) {
    if cmd == "help" {
        std::os::file_write(out_fd, b"Available commands: help, clear, ls, cd, pwd, touch, mkdir, rm, mv, cp, cat\n");
    } else if cmd == "cat" {
        if args.is_empty() {
             // Read from stdin (keyboard or pipe)
             // Note: Simple read loop. For keyboard, this might consume without echo or exit condition (EOF).
             // For pipes, it works until empty (if pipe implementation handles EOF correctly).
        ...
        if args.len() >= 2 {
            ...
            // Copy implementation ... simplified for space
